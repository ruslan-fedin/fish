...
class RouteController
{
    ...
    private function __construct()
    {
        ...
        if ($path === PATH) {
            ...
            if (!$this->routes) throw new RoteException('Сайт находится в техническом обслуживании');

            if (strpos($adress_str, $this->routes['admin']['alias']) === strlen(PATH)) {

                $url = explode('/', substr($adress_str, strlen(PATH . routes['admin']['alias']) + 1));
    ...
    private function createRoute($var,$arr){
        ...
        if (!empty($arr[0])){
            ...
            //Если массив пустой, то используем default
